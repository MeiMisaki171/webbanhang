...
**Date:** 2026-05-14  
**Status:** Approved (brainstorming), revised per storage-gated `auth/me`  
...
**Hệ quả:** Tải API thừa (kể cả `auth/me` cho khách), badge giỏ và menu tài khoản không đồng bộ sau login/logout hoặc mutation giỏ nếu chỉ dựa `router.refresh()`.

## 2. Goals & Non-Goals

### Goals

- Mỗi endpoint `cart` **tối đa một in-flight request** khi bootstrap storefront trong một tab; `auth/me` chỉ gọi khi storage cho thấy có user đã persist để reconcile.
...
3. **Quyết định session từ storage (sau rehydrate):**
   - **Không có `user` persist** (khách hoặc đã logout): **không** gọi `fetchMe()`; đặt `user = null`, `sessionStatus = ready`.
   - **Có `user` persist:** gọi `fetchMe()` đúng một lần để reconcile cookie + profile; thành công → cập nhật `user` + persist; 401 → `user = null`, xóa persist user.
4. **Giỏ hàng:** luôn gọi `fetchCart()` đúng một lần (guest cart cookie); thành công → `set cart`.
5. Cập nhật `cartStatus` (`loading` → `ready` hoặc `error`). `sessionStatus` chỉ `loading` khi đang reconcile `fetchMe()`; khách giữ `ready` ngay sau bước 3.

### Auth

- **Login / register:** gọi API → nhận `AuthUser` → `set user` + persist → `refreshCart()` (phản ánh merge guest cart phía server). Không cần `bootstrap` lại; `refreshSession()` chỉ dùng khi cần reconcile thủ công sau khi đã có user persist.
...
| Tình huống | Hành vi |
|------------|---------|
| Không có `user` trong storage sau rehydrate | Không gọi `auth/me`; `user = null`, `sessionStatus = ready`. |
...
- `bootstrap` không gọi `fetchMe` khi không có `user` persist; vẫn gọi `fetchCart` đúng một lần; dedupe in-flight khi gọi lặp.
...
- Trang chủ, chưa đăng nhập (storage không có user): **không** có request `/auth/me`; đúng **một** request `/cart`.
- Trang chủ, đã đăng nhập (storage có user): đúng **một** request `/auth/me` (reconcile) và **một** `/cart`.
...
- Không còn duplicate mount-driven fetch cho `cart` trên homepage; `auth/me` không gọi cho khách khi storage không có user.
